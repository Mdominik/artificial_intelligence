...
class robot:
    ...
    ############# ONLY ADD/MODIFY CODE BELOW H ERE ###################
    ...
    def move(self, motion): # Do not change the name of this function
        robott = robot()
        distance = motion[1]
        beta = distance / self.length * tan(motion[0])
        if abs(beta) < 0.001:
            ...
            self.orientation = (self.orientation + beta) % (pi*2)
        robott = self
        # ADD CODE HER
        
        return robott # make sure your move function returns an instance
